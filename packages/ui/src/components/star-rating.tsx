...
function Star({ filled, half, size }: { filled: boolean; half: boolean; size: string }) {
      ...
      <svg className={cn(px, "text-amber-400")} viewBox="0 0 20 20">
        <defs>
          <linearGradient id="halfStar">
            ...
            <stop offset="50%" stopColor="rgb(var(--om-line))" />
          </linearGradient>
        </defs>
  ...
  return (
    <svg className={cn(px, filled ? "text-amber-400" : "text-om-line")} viewBox="0 0 20 20" fill="currentColor">
...
export function StarRating({ rating, count, size = "md", className }: StarRatingProps) {
  ...
  return (
    <div className={cn("flex items-center gap-0.5", className)}>
      {stars}
      {count !== undefined && (
        <span className={cn("text-om-ink-soft ml-1", size === "sm" ? "text-xs" : "text-sm")}>
          ({count})
        </span>
      )}
    </div>
